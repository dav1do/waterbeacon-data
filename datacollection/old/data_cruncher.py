...
class SDW_Data_Cruncher(object):
    ...
    def calc_state_scores(self, state, print_test = False):
        ...
        area_scores["FIPSCodes"] = area_scores.apply(lambda x: str(
            x['FIPSCodes']).rstrip('0').rstrip('.').zfill(5), axis=1)
        area_df = pd.merge(area_scores, state_df, left_on='FIPSCodes',
                           right_on='fips_county', how='right')
